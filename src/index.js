...
export default function Tree(container, options) {
    ...
    // if (this.options.selectMode !== 'checkbox' || this.options.selectMode !== 'radio') {
    //     return
    // }
...
Tree.prototype.init = function (data) {
    ...
    let {
        ...
        defaultValues,
        defaultDisables,
    } = Tree.parseTreeData(data, this.options);
    ...
    ele.addEventListener(
        ...
        e => {
            ...
            if (
                ...
                (target.classList.contains(`treejs-${this.options.selectMode}`) ||
...
Tree.prototype.onItemClick = function (id) {
    ...
    if (this.options.selectMode === 'checkbox') {
        if (!node.disabled) {
            this.setValue(id);
...
Tree.parseTreeData = function (data, options) {
    ...
    const walkTree = function (nodes, parent) {
        nodes.forEach(node => {
            ...
            if (options.selectMode === 'checkbox') {
...
Tree.createLiEle = function (node, closed, selectMode) {
    ...
    checkbox.classList.add(`treejs-${selectMode}`);
